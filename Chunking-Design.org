...
repo [[https://github.com/PerseusDLCode/tei-tagger][PerseusDLCode/tei-tagger]].
...
represent physical boundaries — HyperCard cards, manuscript pages, editorial
...
- *Python* (=copy_before(element, stop)=): recursive deep-copy; breaks out of
  the child loop as soon as it encounters =stop= or a subtree containing it.
- *XSLT* (=local:before-stop(node, stop)=): a boolean function checked inline
  in every template — no pre-copy step needed (single-pass).
...
*Note on Python tail rescue:* lxml models text following a child element as a
=.tail= property on that element.  Suppressing =<pb>= with =return None= in
...
Two files in =src/xslt/=, designed for =xsl:import=-based extensibility.

*** =chunker_core.xsl= — importable base

Contains all element templates in *=chunk= mode* plus three helper functions.
All templates receive =$stop= and =$base-urn= as *tunnel parameters*.
...
To extend for a TEI customization: =<xsl:import href="chunker_core.xsl"/>= and
add or override templates.  Import precedence means your overrides automatically
win over the base.
...
Output files are named ={chunk-unit}_{@n}.html=.  An =index.json= manifest is
also written.
...
#+begin_src bash
# Sophocles (Trachiniae), chunked by card
# Greek texts are in data/canonical-greekLit (git submodule)
saxon -s:data/canonical-greekLit/data/tlg0011/tlg001/tlg0011.tlg001.perseus-grc2.xml \
      -xsl:src/xslt/generate_chunks.xsl \
      chunk-unit=card                    \
      output-dir=/tmp/sophocles
#+end_src
...
=div[@type='edition']/@n= in the TEI file.  The citation scheme is declared in
=encodingDesc/refsDecl/cRefPattern=.  Both the Seneca and Sophocles files
...
because =<= is illegal in XML attribute content.  =>>= is technically permitted
unescaped but =&gt;&gt;= is safer.  Both are transparent to the XPath processor
after XML parsing.  Failure to escape =<<= produces a Saxon compile error that
points at =>>=  — a confusing diagnostic.
...
3. Adding a =tei:div[@type='textpart']= template to =chunker_core.xsl=
...
generating customization-specific stylesheets derived from an ODD.  The design
for this has not yet been worked out.

*** Tests

No automated tests exist for the XSLT.  The Python module has a test directory
but no substantive test cases.

** Source history

| Repo                                                              | Role                                                              |
|-------------------------------------------------------------------+-------------------------------------------------------------------|
| [[https://github.com/PerseusDLCode/tei-tagger][PerseusDLCode/tei-tagger]] | Scratchpad — full development history including XQuery explorations |
...
| =5ef6a40= | XSLT batch generator (=generate_chunks.xsl= + =chunker_core.xsl=) |
